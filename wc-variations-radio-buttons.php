<?php
/**
 * Plugin Name: Woocommerce Variations Switch tiles
 * Plugin URI:  TBA
 * Description: Forked from Variations Radio Buttons for WooCommerce. Let your customers choose product variations using rectangular switch tile buttons instead of dropdowns. Also includes SVG icons. This version is hardcoded for a specific store, loose plans include universalising the styles and svg inputs.
 * Version:     2.2.0
 * Author:      Lasse Jellum
...
// HPOS compatibility must be declared before WC initialises, so register the
// hook at load time rather than waiting for plugins_loaded.
...
if ( ! class_exists( 'WC_Radio_Buttons' ) ) {
	...
	class WC_Radio_Buttons {
		...
		public function locate_template( $template, $template_name, $template_path ) {
			...
			// Plugin template if no theme override.
			if ( ! $template && file_exists( $plugin_path . $template_name ) ) {
				$template = $plugin_path . $template_name;
			}

			// Fall back to WC default.
			if ( ! $template ) {
				$template = $_template;
			}
		...
		/**
		 * Enqueue assets on product pages only. Non-product requests skip the
		 * whole path so we don't deregister scripts or emit SVG defs site-wide.
		 */
		public function load_scripts() {
			...
			// Bundle products have their own switcher; leave their JS alone.
			if ( ! ( $product && $product->is_type( 'bundle' ) ) ) {
				wp_deregister_script( 'wc-add-to-cart-variation' );
				wp_register_script(
					'wc-add-to-cart-variation',
					$this->get_plugin_url() . 'assets/js/frontend/add-to-cart-variation.js',
		...
		/**
		 * SVG symbol definitions consumed by the switch tiles. Rendered once
		 * per product page footer — not on every site page.
		 */
